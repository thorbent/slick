...
trait BasicProfile extends BasicInvokerComponent with BasicExecutorComponent { driver: BasicDriver =>
  ...
  trait Implicits extends ExtensionMethodConversions {
    ...
    implicit def queryToQueryExecutor[E, U, C[_]](q: Query[E, U, C]): QueryExecutor[C[U]] = createQueryExecutor[C[U]](queryCompiler.run(q.toNode).tree, ())
    implicit def shapedValueToQueryExecutor[T, U](u: ShapedValue[T, U]): QueryExecutor[U] = createQueryExecutor[U](queryCompiler.run(u.toNode).tree, ())
