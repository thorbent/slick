...
trait JdbcProfile extends SqlProfile with JdbcTableComponent
  ...
  trait LowPriorityImplicits {
    implicit def queryToAppliedQueryInvoker[T, U, C[_]](q: Query[T, _ <: U, C]): UnitQueryInvoker[U] = createUnitQueryInvoker[U](queryCompiler.run(q.toNode).tree)
    implicit def queryToUpdateInvoker[E, U, C[_]](q: Query[E, U, C]): UpdateInvoker[U] = createUpdateInvoker(updateCompiler.run(q.toNode).tree, ())
  }
  ...
  trait Implicits extends LowPriorityImplicits with super.Implicits with ImplicitColumnTypes {
    ...
    implicit def runnableCompiledToAppliedQueryInvoker[RU, C[_]](c: RunnableCompiled[_ <: Query[_, _, C], Seq[RU]]): MutatingUnitInvoker[RU] = createQueryInvoker[Any, RU](c.compiledQuery)(c.param)
    implicit def runnableCompiledToUpdateInvoker[RU, C[_]](c: RunnableCompiled[_ <: Query[_, _, C], Seq[RU]]): UpdateInvoker[RU] =
      createUpdateInvoker(c.compiledUpdate, c.param)
    implicit def runnableCompiledToDeleteInvoker[RU, C[_]](c: RunnableCompiled[_ <: Query[_, _, C], Seq[RU]]): DeleteInvoker =
